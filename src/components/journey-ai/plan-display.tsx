...
import { PlanActionsBar } from '@/components/journey-ai/plan-actions-bar';
...
const getIconForActivityType = (type: string) => {
  ...
  if (
    ...
    lowerType.includes('landmark') ||
    lowerType.includes('gallery')
  )
    return <Landmark className="w-4 h-4 mr-2 text-primary shrink-0" />;
  if (
    ...
    return <Coffee className="w-4 h-4 mr-2 text-primary shrink-0" />;
  if (
    ...
    return <Sun className="w-4 h-4 mr-2 text-primary shrink-0" />;
  if (lowerType.includes('shop') || lowerType.includes('market'))
    return <Wallet className="w-4 h-4 mr-2 text-primary shrink-0" />;
  ...
  if (
    ...
    return <Users className="w-4 h-4 mr-2 text-primary shrink-0" />;
  return <Info className="w-4 h-4 mr-2 text-primary shrink-0" />;
};

export function PlanDisplay({ plan, destinationName, onOpenSavedPlans }: PlanDisplayProps) {
  ...
  const totalItems = plan.packingList?.length || 0;
      ...
      <Card className="shadow-xl print:shadow-none print:border-none">
        <CardHeader className="bg-secondary/30">
          <CardTitle className="text-2xl md:text-3xl font-headline text-primary">
            {plan.tripTitle || `Your Trip to ${destinationName}`}
          </CardTitle>
          {plan.overallSummary && (
            <CardDescription className="text-base mt-1">{plan.overallSummary}</CardDescription>
          )}
        </CardHeader>
        <CardContent className="space-y-8 p-4 md:p-6">
                  ...
                  >
                    <AccordionTrigger className="px-4 py-3 hover:bg-secondary/50 rounded-t-lg">
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between w-full text-left">
                        ...
                        {day.date && !day.theme && (
                          <Badge variant="outline" className="mt-1 sm:mt-0 sm:ml-2">
                            {new Date(day.date + 'T00:00:00').toLocaleDateString(undefined, {
                              month: 'short',
                              day: 'numeric',
                            })}
                    ...
                    <AccordionContent className="px-4 pb-4 pt-2 space-y-4">
                      {day.dailySummary && (
                        <p className="text-muted-foreground mb-4 italic">{day.dailySummary}</p>
                      )}

                      {day.morningActivities?.length > 0 && (
                        <div className="mb-3">
                          <h4 className="text-md font-semibold flex items-center mb-2 text-foreground/90">
                            ...
                            {day.morningActivities.map((activity, actIndex) => (
                              ...
                              >
                                <strong className="flex items-center">
                                  {getIconForActivityType(activity.type)} {activity.name}
                                </strong>{' '}
                                ({activity.type})
                                {activity.estimatedDuration && (
                                  <small className="text-muted-foreground block ml-6">
                                    Est. Duration: {activity.estimatedDuration}
                                  </small>
                                )}
                                <p className="text-sm text-muted-foreground ml-6 my-1">
                                  {activity.description}
                                </p>
                                {activity.address && (
                                  <small className="text-xs text-muted-foreground/80 flex items-center ml-6">
                                    <MapPin className="w-3 h-3 mr-1" />
                                    {activity.address}
                                  </small>
                                )}
                                {activity.notes && (
                                  <small className="text-xs text-blue-600 block mt-1 ml-6">
                                    Note: {activity.notes}
                                  </small>
                                )}
                              </li>
            ...
            <div className="space-y-4">
              {plan.transportationDetails?.gettingToDestination && (
                <div>
                  ...
                  <FormattedMarkdownText
                    text={plan.transportationDetails.gettingToDestination}
              ...
              {plan.transportationDetails?.interCityTravel && (
                <div>
                  ...
                  <FormattedMarkdownText
                    text={plan.transportationDetails.interCityTravel}
                    className="text-sm text-muted-foreground bg-secondary/30 p-3 rounded-md"
                  />
                </div>
              )}
              {plan.transportationDetails?.localTransportInDestination && (
                <div>
                  <h3 className="text-lg font-medium flex items-center mb-1 text-foreground/90">
                    <Bus className="w-5 h-5 mr-2 text-primary" /> Local Transport in{' '}
                    {destinationName}
                  </h3>
                  <FormattedMarkdownText
                    text={plan.transportationDetails.localTransportInDestination}
                    className="text-sm text-muted-foreground bg-secondary/30 p-3 rounded-md"
                  />
                </div>
              )}
          ...
          {plan.accommodationRecommendations && plan.accommodationRecommendations.length > 0 && (
            <div>
              ...
              <ul className="list-disc list-inside text-sm text-muted-foreground bg-secondary/30 p-3 rounded-md space-y-1">
                {plan.accommodationRecommendations.map((rec, index) => (
                  <li key={index}>{rec}</li>
                ))}
              </ul>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <Badge
                variant="secondary"
                className="text-lg px-3 py-1 bg-accent/10 text-accent-foreground border-accent"
              >
                {plan.currency} {plan.estimatedCost.toLocaleString()}
              </Badge>
              <p className="text-xs text-muted-foreground mt-1">
                (Excludes travel to destination from source)
              </p>
            </div>
              ...
              <ScrollArea className="h-48 rounded-md border p-2">
                <div className="space-y-1.5 p-1">
                  {plan.packingList.map((item, index) => {
                    const isChecked = Boolean(packedItems[index]);
                    return (
                      <div
                        key={index}
            ...
            <div>
              ...
              <ScrollArea className="h-32 rounded-md border p-1">
                <ul className="list-disc list-inside text-sm text-muted-foreground p-3 space-y-1">
                  {plan.localCustomsOrTips.map((tip, index) => (
                    <li key={index}>{tip}</li>
                  ))}
                </ul>
              </ScrollArea>
