...
const GenerateLocalTravelSuggestionsInputSchema = z.object({
  ...
  startTime: z.string().regex(/^([01]\d|2[0-3]):([0-5]\d)$/, 'Start time must be in HH:MM (24-hour) format.').describe('Start time for activities in HH:MM (e.g., "09:00").'),
  endTime: z.string().regex(/^([01]\d|2[0-3]):([0-5]\d)$/, 'End time must be in HH:MM (24-hour) format.').describe('End time for activities in HH:MM (e.g., "18:00").'),
  modeOfTransport: z.enum(['public', 'private', 'own', 'walking', 'cycling', 'any']).describe('Preferred mode of transportation.'),
  preferences: z.string().optional().describe('User preferences or interests (e.g., "art museums, coffee shops, budget-friendly").'),
});
export type GenerateLocalTravelSuggestionsInput = z.infer<typeof GenerateLocalTravelSuggestionsInputSchema>;
...
const SuggestedActivitySchema = z.object({
  ...
  estimatedDuration: z.string().describe('Estimated time to spend (e.g., "1-2 hours"). Critical for fitting into user time window.'),
  address: z.string().optional().describe('Full address or specific location.'),
  reasonWhySuggested: z.string().optional().describe('Specific reason why this fits user criteria.'),
  openingHours: z.string().optional().describe('Relevant opening hours for {{{date}}} (e.g., "10 AM - 6 PM"). MUST be open during user\'s {{{startTime}}}-{{{endTime}}} window on {{{date}}}. If unsure, state "Verify opening hours".'),
  estimatedCost: z.string().optional().describe('Estimated cost (e.g., "Free", "$10-20").'),
  bookingNeeded: z.string().optional().describe('Booking: "Recommended", "Required", or "Not usually needed".'),
...
const GenerateLocalTravelSuggestionsOutputSchema = z.object({
  title: z.string().describe("Catchy, personalized title for the plan (e.g., 'Your [StartTime]-[EndTime] in [Location]')."),
  introduction: z.string().optional().describe('Brief intro paragraph (2-3 sentences) for the plan.'),
  suggestedItinerary: z.array(SuggestedActivitySchema).min(1).max(5).describe('List of 1 to 5 suggested activities. MUST fit within user\'s time window {{{startTime}}}-{{{endTime}}}, including travel.'),
  overallTimeManagementNotes: z.string().optional().describe("Notes on fitting activities into time window. If sequential, MUST include estimated travel time AND mode between them. Sum durations and travel, state if fits user's available time from {{{startTime}}} to {{{endTime}}}. Mention buffer time."),
  transportationAdvice: z.string().optional().describe("General advice for using {{{modeOfTransport}}} (or alternatives if 'any') in {{{location}}}. Markdown format: bold heading per mode (e.g., '**Taxis:**'), then bullet points for details (availability, cost, how to use). No specific inter-activity travel here."),
  alternativeSuggestion: SuggestedActivitySchema.optional().describe("One alternative activity fitting criteria."),
});
export type GenerateLocalTravelSuggestionsOutput = z.infer<typeof GenerateLocalTravelSuggestionsOutputSchema>;

export async function generateLocalTravelSuggestions(input: GenerateLocalTravelSuggestionsInput): Promise<GenerateLocalTravelSuggestionsOutput> {
